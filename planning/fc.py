...
def read_planning_fc_targets(workbook_bytes):
    ...
    try:
        ...
        header = next(
            fc_sheet.iter_rows(
                min_row=FC_HEADER_ROW,
                max_row=FC_HEADER_ROW,
                min_col=FC_HEADER_MIN_COL,
                max_col=FC_HEADER_MAX_COL,
                values_only=True,
            ),
            (),
        )
        matched_columns = [
            FC_HEADER_MIN_COL + offset
            for offset, value in enumerate(header)
            if _normalize_header(value) == selector_key
        ]
        if not matched_columns:
            raise RuntimeError(
                f"{FC_SHEET}!{FC_SELECTOR_CELL}={selector!r} không khớp "
                f"tiêu đề nào trong {FC_SHEET}!E1:P1."
            )
        ...
        for row_number, values in enumerate(
            fc_sheet.iter_rows(
                min_row=2,
                max_col=max(FC_CODE_COL, source_column),
                values_only=True,
            ),
            start=2,
        ):
            code = normalize_code(values[FC_CODE_COL - 1])
            if not code:
                continue
            if code in fc_values:
                raise RuntimeError(f"Mã {code} bị lặp trong {FC_SHEET}!B.")
            fc_values[code] = _forecast_value(
                values[source_column - 1],
                f"{FC_SHEET}!{source_column_letter}{row_number}",
            )

        if not fc_values:
            raise RuntimeError(f"Không đọc được mã sản phẩm trong {FC_SHEET}!B.")

        planning = values_workbook[PLANNING_SHEET]
        targets = {}
        seen_codes = set()
        missing_codes = []
        changed_count = 0
        ...
        for row_number, values in enumerate(
            planning.iter_rows(
                min_row=2,
                max_col=PLANNING_TARGET_COL,
                values_only=True,
            ),
            start=2,
        ):
            code = normalize_code(values[PLANNING_CODE_COL - 1])
            if not code:
                continue
            ...
            current_value = values[PLANNING_TARGET_COL - 1]
            if not _values_equal(current_value, target_value):
                changed_count += 1
