...
def prepare_calendar_update_all_months(workbook_bytes, *, plan_year=None):
    ...
    if plan_year is None:
        with zipfile.ZipFile(BytesIO(workbook_bytes), "r") as archive:
            selector = _read_selector_from_archive(archive)
        plan_month = parse_plan_month(selector)
        plan_year = resolve_plan_year(plan_month)

    updated, info = prepare_calendar_update(workbook_bytes, plan_year=plan_year)
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if info["changed_count"] == 0:
                print(
                    f"[{PLANNING_SHEET}] Dải ngày đã đúng; không cần upload lại."
                )
                return info
